...
/*
  ...
  modules should appear.  Click on the row for the Ground Delivery Zone Rates Module.
...
//		echo $dest_zone;
      ...
      if ($dest_zone == 0) {
        ...
        $done = false;
        for ($i=0; $i<$size; $i+=2) {
          switch (MODULE_SHIPPING_ZIPCODEZONE_METHOD) {
        	  case (MODULE_SHIPPING_ZIPCODEZONE_METHOD == 'Weight'):
...
//                $shipping_method = MODULE_SHIPPING_ZIPCODEZONE_TEXT_WAY . ' ' . $dest_country . (SHIPPING_BOX_WEIGHT_DISPLAY >= 2 ? ' : ' . $shipping_weight . ' ' . MODULE_SHIPPING_ZIPCODEZONE_TEXT_UNITS : '');
                $shipping_method = MODULE_SHIPPING_ZIPCODEZONE_TEXT_WAY . ' ' . $order->delivery['postcode'] . $show_box_weight;
                $done = true;
        if (strstr($zones_table[$i+1], '%')) {
          ...
          $shipping = $zones_table[$i+1];
      	    ...
      	    break;
        	  case (MODULE_SHIPPING_ZIPCODEZONE_METHOD == 'Price'):
// shipping adjustment
        ...
        if (strstr($zones_table[$i+1], '%')) {
          ...
          $shipping = $zones_table[$i+1];
	          ...
	          break;
        	  case (MODULE_SHIPPING_ZIPCODEZONE_METHOD == 'Item'):
// shipping adjustment
      ...
      if (strstr(MODULE_SHIPPING_ZIPCODEZONE_SKIPPED, $dest_zone)) {
        // don't show anything for this country
        $this->quotes = array();
      } else {
        if ($error == true) $this->quotes['error'] = MODULE_SHIPPING_ZIPCODEZONE_INVALID_ZONE;
      }
