 ...
 * Modified for zipcodezone usage by John Taylor - http://www.greatcanada.biz
 * 1/14/2018 - for Zen-Cart 1.5.5 by tmccaff
 */
/*

  USAGE
  By default, the module comes with support for 7 zones numbered 2 - 8.  This can be
  easily changed by editing the lines below in the zones constructor 
  that defines:
  $this->num_zones - TOTAL NUMBER OF ZONES (NOT THE END COUNT)
  $this->num_zone_start - WHICH ZONE NUMBER TO START AT (in this case it is 2)
  ...
  modules should appear.  Click on the row for the Rush Delivery Zone Rates Module.
  You should see an install button.  Click install and a list of settings
  will appear to the right.  Click on the Edit button. 
  ...
  The contribution comes with a Default MYSQL Patch for the 8 Delivery Zones in the US.
  Go to Admin and go into tools and then enter the patch into the MYSQL Patch as is.
  
  If you wish to add countries you will need to add this to the data base either by 
  phpmyadmin or by updating the patch file.  At this time there is no file to add 
  zone ranges.
...
// CUSTOMIZE THIS SETTING FOR THE NUMBER OF ZONES NEEDED
      ...
      $this->sort_order = MODULE_SHIPPING_ZIPRUSH_SORT_ORDER;
      $this->icon = '';
      ...
      $this->enabled = ((MODULE_SHIPPING_ZIPRUSH_STATUS == 'True') ? true : false);
        ...
        if ( ($this->enabled == true) && ((int)MODULE_SHIPPING_ZIPRUSH_ZONE > 0) ) {
            $check_flag = false;
      $check = $db->Execute("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . "
                             where geo_zone_id = '" . MODULE_SHIPPING_ZIPRUSH_ZONE . "'
      ...
      if ($this->enabled) {
        global $db;
          ...
          $check_query = $db->Execute("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_SHIPPING_ZIPRUSH_HANDLING_METHOD_" . $i . "'");
          if ($check_query->EOF) {
...
//		echo $dest_zone;
      ...
      if ($dest_zone == 0) {
        ...
        $shipping = -1;
        $zones_cost = constant('MODULE_SHIPPING_ZIPRUSH_COST_' . $dest_zone);

        $zones_table = preg_split("/[:,]/" , $zones_cost);
        $size = sizeof($zones_table);
        $done = false;
        for ($i=0; $i<$size; $i+=2) {
          switch (MODULE_SHIPPING_ZIPRUSH_METHOD) {
        	  case (MODULE_SHIPPING_ZIPRUSH_METHOD == 'Weight'):
              if (round($shipping_weight,9) <= $zones_table[$i]) {
                ...
                case (1):
                  ...
                  break;
                case (2):
                  $show_box_weight = ' (' . number_format($shipping_weight * $shipping_num_boxes,2) . MODULE_SHIPPING_ZIPRUSH_TEXT_UNITS . ')';
                  break;
                default:
                  $show_box_weight = ' (' . $shipping_num_boxes . ' x ' . number_format($shipping_weight,2) . MODULE_SHIPPING_ZIPRUSH_TEXT_UNITS . ')';
                  break;
                }

//                $shipping_method = MODULE_SHIPPING_ZIPRUSH_TEXT_WAY . ' ' . $dest_country . (SHIPPING_BOX_WEIGHT_DISPLAY >= 2 ? ' : ' . $shipping_weight . ' ' . MODULE_SHIPPING_ZIPRUSH_TEXT_UNITS : '');
                $shipping_method = MODULE_SHIPPING_ZIPRUSH_TEXT_WAY . ' ' . $order->delivery['postcode'] . $show_box_weight;
                $done = true;
        if (strstr($zones_table[$i+1], '%')) {
          ...
          $shipping = $zones_table[$i+1];
      	    ...
      	    break;
        	  case (MODULE_SHIPPING_ZIPRUSH_METHOD == 'Price'):
// shipping adjustment
              if (($_SESSION['cart']->show_total() - $_SESSION['cart']->free_shipping_prices()) <= $zones_table[$i]) {
                $shipping = $zones_table[$i+1];
                $shipping_method = MODULE_SHIPPING_ZIPRUSH_TEXT_WAY . ' ' . $order->delivery['postcode'];
        ...
        if (strstr($zones_table[$i+1], '%')) {
          ...
          $shipping = $zones_table[$i+1];
	          ...
	          break;
        	  case (MODULE_SHIPPING_ZIPRUSH_METHOD == 'Item'):
// shipping adjustment
        ...
        if ($shipping == -1) {
          $shipping_cost = 0;
          $shipping_method = MODULE_SHIPPING_ZIPRUSH_UNDEFINED_RATE;
        } else {
          switch (MODULE_SHIPPING_ZIPRUSH_METHOD) {
        	  case (MODULE_SHIPPING_ZIPRUSH_METHOD == 'Weight'):
              // charge per box when done by Weight
              // Handling fee per box or order
              if (constant('MODULE_SHIPPING_ZIPRUSH_HANDLING_METHOD_' . $dest_zone) == 'Box') {
                $shipping_cost = ($shipping * $shipping_num_boxes) + constant('MODULE_SHIPPING_ZIPRUSH_HANDLING_' . $dest_zone) * $shipping_num_boxes;
              } else {
                $shipping_cost = ($shipping * $shipping_num_boxes) + constant('MODULE_SHIPPING_ZIPRUSH_HANDLING_' . $dest_zone);
              }
              break;
        	  case (MODULE_SHIPPING_ZIPRUSH_METHOD == 'Price'):
              // don't charge per box when done by Price
              $shipping_cost = ($shipping) + constant('MODULE_SHIPPING_ZIPRUSH_HANDLING_' . $dest_zone);
            break;
        	  case (MODULE_SHIPPING_ZIPRUSH_METHOD == 'Item'):
      ...
      $this->quotes = array('id' => $this->code,
                            'module' => MODULE_SHIPPING_ZIPRUSH_TEXT_TITLE,
      ...
      if (strstr(MODULE_SHIPPING_ZIPRUSH_SKIPPED, $dest_zone)) {
        // don't show anything for this country
        $this->quotes = array();
      } else {
        if ($error == true) $this->quotes['error'] = MODULE_SHIPPING_ZIPRUSH_INVALID_ZONE;
      }
    ...
    function check() {
      ...
      if (!isset($this->_check)) {
        $check_query = $db->Execute("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_SHIPPING_ZIPRUSH_STATUS'");
    ...
    function install() {
      global $db;
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Enable Zones Method', 'MODULE_SHIPPING_ZIPRUSH_STATUS', 'True', 'Do you want to offer zone rate shipping?', '6', '0', 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Calculation Method', 'MODULE_SHIPPING_ZIPRUSH_METHOD', 'Weight', 'Calculate cost based on Weight, Price or Item?', '6', '0', 'zen_cfg_select_option(array(\'Weight\', \'Price\', \'Item\'), ', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Tax Class', 'MODULE_SHIPPING_ZIPRUSH_TAX_CLASS', '0', 'Use the following tax class on the shipping fee.', '6', '0', 'zen_get_tax_class_title', 'zen_cfg_pull_down_tax_classes(', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Tax Basis', 'MODULE_SHIPPING_ZIPRUSH_TAX_BASIS', 'Shipping', 'On what basis is Shipping Tax calculated. Options are<br />Shipping - Based on customers Shipping Address<br />Billing Based on customers Billing address<br />Store - Based on Store address if Billing/Shipping Zone equals Store zone', '6', '0', 'zen_cfg_select_option(array(\'Shipping\', \'Billing\', \'Store\'), ', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_SHIPPING_ZIPRUSH_SORT_ORDER', '0', 'Sort order of display.', '6', '0', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Shipping Zone', 'MODULE_SHIPPING_ZIPRUSH_ZONE', '0', 'If a zone is selected, only enable this shipping method for that zone.', '6', '0', 'zen_get_zone_class_title', 'zen_cfg_pull_down_zone_classes(', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Skip Countries, use a comma separated list of the two character ISO country codes', 'MODULE_SHIPPING_ZIPRUSH_SKIPPED', '', 'Disable for the following Countries:', '6', '0', 'zen_cfg_textarea(', now())");
      ...
      for ($i = $this->num_zone_start; $i <= $this->num_zones + ($this->num_zone_start - 1); $i++) {
        ...
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Zone " . $i ." Shipping Table', 'MODULE_SHIPPING_ZIPRUSH_COST_" . $i ."', '3:8.50,7:10.50,99:20.00', 'Shipping rates to Zone " . $i . " destinations based on a group of maximum order weights/prices. Example: 3:8.50,7:10.50,... Weight/Price less than or equal to 3 would cost 8.50 for Zone " . $i . " destinations.<br />You can also use percentage amounts, such 25:8.50,35:5%,40:9.50,10000:7% to charge a percentage value of the Order Total', '6', '0', 'zen_cfg_textarea(', now())");
    ...
    function remove() {
      global $db;
      $db->Execute("delete from " . TABLE_CONFIGURATION . " where configuration_key like 'MODULE\_SHIPPING\_ZIPRUSH\_%'");
    }

    function keys() {
      $keys = array('MODULE_SHIPPING_ZIPRUSH_STATUS', 'MODULE_SHIPPING_ZIPRUSH_METHOD', 'MODULE_SHIPPING_ZIPRUSH_TAX_CLASS', 'MODULE_SHIPPING_ZIPRUSH_TAX_BASIS', 'MODULE_SHIPPING_ZIPRUSH_ZONE', 'MODULE_SHIPPING_ZIPRUSH_SORT_ORDER', 'MODULE_SHIPPING_ZIPRUSH_SKIPPED');

      for ($i = $this->num_zone_start; $i <= $this->num_zones + ($this->num_zone_start - 1); $i++) {
        $keys[] = 'MODULE_SHIPPING_ZIPRUSH_COUNTRIES_' . $i;
        $keys[] = 'MODULE_SHIPPING_ZIPRUSH_COST_' . $i;
        $keys[] = 'MODULE_SHIPPING_ZIPRUSH_HANDLING_' . $i;
        $keys[] = 'MODULE_SHIPPING_ZIPRUSH_HANDLING_METHOD_' . $i;
      }
